...
class SouceTemplateTestCase(unittest.TestCase):
    ...
    def test_load_template(self):
        source_template = Source("tests/testfiles/template.xlsx", "tests/testfiles/data.xlsx")
    ...
    def test_except_load_template(self):
        ...
        with self.assertRaises(SourceTemplateNotLoadError):
            Source("tests/testfiles/result.txt", "tests/testfiles/data.xlsx")
    def test_except_load_data(self):

        with self.assertRaises(SourceDataNotLoadError):
            Source("tests/testfiles/template.xlsx", "tests/testfiles/result.txt")
...
class TextRenderTestCase(unittest.TestCase):
    ...
    def setUp(self):
        source = Source("tests/testfiles/template.xlsx", "tests/testfiles/data.xlsx")
        self.data_map = source.parse()
        # print(self.data_map)

    def test_isinstance_base(self):
        textrender = TextRender("tests/testfiles/export_template.html", self.data_map)
        self.assertIsInstance(textrender, BaseRender)

    def test_rendering(self):
        with open("tests/testfiles/result.txt", encoding="utf-8") as equal_text_file:
